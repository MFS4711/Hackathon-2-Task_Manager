...
class TaskModelTest(TestCase):
    ...
    def test_task_creation(self):
        ...
        `priority`, `status`, `category`, and `due_date` are properly set
    ...
    def test_invalid_priority(self):
        ...
        for a `Task`, a `ValidationError` should be raised when the object
        is validated.
        """
        task = Task(
            user=self.user,
    ...
    def test_task_due_date(self):
        ...
        This test checks whether the `due_date` is a valid future date
        when set, and ensures that a task with an invalid date
        (e.g., a past date) should not be created.
        """
    ...
    def test_task_overdue_status(self):
        ...
        if the task's due date is in the past and the status hasn't been marked
        as completed.
        """
    ...
    def test_task_category_choices(self):
        ...
        This test validates that the `category` field in the model respects the
        choices defined.
        """
        valid_categories = [choice[0] for choice in Task.CATEGORY_CHOICES]
        self.assertIn(self.task.category, valid_categories)
    ...
    def test_task_user_association(self):
        ...
        This test ensures that the `user` field links the task to the correct
        `User`.
        """
    ...
    def test_task_status_completed_requires_in_progress(self):
        ...
        task = Task.objects.create(
            ...
            description="A task that's being directly set as 'Completed' \
            without being 'In Progress'",
            priority=Task.MEDIUM,
    ...
    def test_task_status_not_marked_overdue_if_completed(self):
        """
        Test that a Task marked as 'Completed' is not automatically set to
        'Overdue' even if the due date is in the past.
        """
        ...
        # Ensure that status remains 'Completed' and not changed to 'Overdue'
    ...
    def test_task_status_completed_to_in_progress(self):
        """
        Test that a task marked as 'Completed' cannot be changed back to
        'In Progress'.
        """
        task = Task.objects.create(
            user=self.user,
    ...
    def test_task_status_cannot_be_set_to_overdue_manually(self):
        ...
        # Expect ValidationError - status should not be manually set 'Overdue'
        with self.assertRaises(ValidationError):
